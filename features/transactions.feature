...
Feature: Transaction handling
  ...
  Scenario: Complete a dynamic deposit via iframe token
    ...
    And the user's EUR balance has increased by 75.50

  Scenario: A deposit with a malformed amount is refused and leaves the balance alone
    Given an iframe token obtained with scope ["deposit"] and mapped to the managed user
    And I record the user's EUR balance
    When I POST /transaction/complete?paymentType=deposit&bearer={iframeToken} with amount "12,50" and currency "EUR" and Authorization header "Bearer {iframeToken}"
  ...
  # The two scenarios above cover the balance direction. This one covers the
  # response: a consumer reconciling a transfer needs the sending side echoed
  # back, which the transaction response did not previously carry.
  Scenario: A hosted transfer echoes back the wallet the money left
    Given the user has a funded EUR balance
    When I POST a hosted transfer of 25.00 EUR from the user wallet to "rExternalSettlement"
    Then the response status is 201
    And the response reports sending_address as the user wallet
